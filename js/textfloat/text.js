function Neontext(id) {
	...
	this.spanArr = (function() {
		...
		arr.forEach(function(value, index) {
			...
			spanDom.className = "word"+index;
			spanDom.style.position = "absolute";
			spanDom.style.transition = "0.7s";
			spanDom.innerHTML = value;
			spanDom.style.zIndex = ele.hasAttribute("ele-range") ? "1" : "-1";
			spanDom.style.color = _this.randomColor();
			var randomW = parseInt($(window).width()*Math.random());
			var randomH = parseInt($(window).height()*Math.random());
			spanDom.style.transform = "translateX(" + randomW + "px) translateY(" + randomH + "px)";
			spanDom.own = {
				pos: {
					x: 0,
					y: 0
